...
export const listingService = {
  ...
  getAvailable: async (): Promise<Listing[]> => {
    const response = await api.get('/listings');
    return response.data;
  },

  //Get listings created by a specific branch
  getByBranch: async (branchId: string): Promise<Listing[]> => {
    const response = await api.get('/get_listings', {
  ...
  //Update listing line items
  updateItem: async (data: { user_branch_id: string; listing_id: string; items: UpdateListingItemRequest[] }): Promise<void> => {
    await api.patch('/listing/items', data);
  },
