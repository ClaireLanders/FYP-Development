// Custom hook for managing listings (edit/cancel operations)
// Fetches listings for a specific branch from the backend
// Provides functions to update individual items and cancel entire listings
// Manages loading state and error handling
// Automatically refetches data after mutations

import { useState, useEffect } from 'react';
import { listingService } from '../services/listingService';
import type { Listing } from '../services/types';

export const useListingManagement = (branchId: string, userBranchId: string) => {
  ...
  const fetchListings = async () => {
    try {
      setLoading(true);
      setError(null);
      const data = await listingService.getByBranch(branchId);
      setListings(data);
    } catch (err) {
  ...
  const updateItem = async (listingId: string, listingLineItemId: string, quantity: number) => {
    try {
      await listingService.updateItem({
        user_branch_id: userBranchId,
        listing_id: listingId,
        items: [{ listing_line_item_id: listingLineItemId, quantity }],
      });
  ...
  return { listings, loading, error, refetch: fetchListings, updateItem, cancelListing };
};
